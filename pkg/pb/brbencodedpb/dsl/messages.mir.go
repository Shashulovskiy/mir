...
func UponEchoMessageReceived(m dsl.Module, handler func(from types1.NodeID, hash []uint8, chunk []uint8) error) {
	UponMessageReceived[*types.Message_EchoMessage](m, func(from types1.NodeID, msg *types.EchoMessage) error {
		return handler(from, msg.Hash, msg.Chunk)
	})
}

func UponReadyMessageReceived(m dsl.Module, handler func(from types1.NodeID, hash []uint8, chunk []uint8) error) {
