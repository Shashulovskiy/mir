...
func UponStartMessageReceived(m dsl.Module, handler func(from types1.NodeID, id int64, n int64, chunk []uint8, rootHash []uint8, proof *commonpb.MerklePath) error) {
	UponMessageReceived[*types.Message_StartMessage](m, func(from types1.NodeID, msg *types.StartMessage) error {
		return handler(from, msg.Id, msg.N, msg.Chunk, msg.RootHash, msg.Proof)
	})
}

func UponEchoMessageReceived(m dsl.Module, handler func(from types1.NodeID, id int64, n int64, chunk []uint8, rootHash []uint8, proof *commonpb.MerklePath) error) {
	UponMessageReceived[*types.Message_EchoMessage](m, func(from types1.NodeID, msg *types.EchoMessage) error {
		return handler(from, msg.Id, msg.N, msg.Chunk, msg.RootHash, msg.Proof)
	})
}

func UponReadyMessageReceived(m dsl.Module, handler func(from types1.NodeID, id int64, n int64, rootHash []uint8) error) {
	UponMessageReceived[*types.Message_ReadyMessage](m, func(from types1.NodeID, msg *types.ReadyMessage) error {
		return handler(from, msg.Id, msg.N, msg.RootHash)
	})
}
